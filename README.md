...
**Sample weekly report excerpt:**

```
Campaign execution health is moderate this week. Campaign readiness stands at
83.8%, with sell-through at 64.9% against a 70.0% target. There are 1,603 open
exceptions (651 critical), of which 160 have breached SLA. Inventory risk is
...
- **Simulated data only.** No real company data, no real store locations, no
  real sales figures. All data is synthetic and deterministic.
- **No deep learning.** The project uses rule-based logic and deterministic
  formulas. No neural networks, no ML model training, no time-series
  forecasting.
- **No ERP clone.** The project models the retail ops control-tower workflow
  only. It does not replicate order management, warehouse management, financial
  accounting, HR, or procurement systems.
- **No authentication.** Single-user application. No login, no role-based access
  control.
- **No real photos.** Photo proof records contain metadata (GPS, timestamp,
  user ID) but no actual images are stored or processed.
- **No real-time streaming.** Data is generated in batches.
- **No cloud deployment.** The project runs locally.
- **No third-party API integrations.** All data is internally generated.
- **Not published to PyPI.** Install from source only.
- **Not on GitHub (yet).** Publishing is a separate approval gate.

Deferred to a future version (documented but not built):

- WSSI (Weekly Sales, Stock and Intake) trading view.
- Transfer and markdown optimization engine.
- Performance KPIs requiring POS data (sales vs. target, conversion, ATV).
- Per-role action list filtering (7 roles).
- Sub-component compliance tiles (planogram, POSM, pricing as separate tiles).
...
|-- artifacts/                             # Portfolio narrative and CV materials
```
